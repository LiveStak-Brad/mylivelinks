...
function ConsentContent() {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/auth/v1/oauth/authorize`, {
        method: 'POST',
        headers: {
      ...
      if (res.ok) {
        const redirectTo: string | undefined = data.redirect_to || data.redirectTo || data.url;
        if (redirectTo) {
      ...
      const msg = data?.error_description || data?.error || 'We could not complete the authorization. Please try again.';
      setError(msg);
    } catch (err: any) {
        ...
        <Card className="shadow-xl border-0">
          <CardHeader className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription>Approve access so this app can use your MyLiveLinks account.</CardDescription>
            </div>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            {userEmail && (
              <div className="flex flex-col gap-1 text-sm text-muted-foreground">
                ...
                <div className="text-xs">If this is not you, please sign out and sign back in with the correct account.</div>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <Button variant="outline" onClick={handleDeny} disabled={!redirectUri || status === 'loading'}>
                Deny
              </Button>
              <Button
