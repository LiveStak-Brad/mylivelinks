...
export async function GET(request: NextRequest) {
  try {
    ...
    const clipId = url.searchParams.get('p_clip_id');

    if (!clipId) {
      return NextResponse.json({ error: 'p_clip_id is required' }, { status: 400 });
    }
    ...
    console.error('GET /api/get_clip_playback error:', err);
