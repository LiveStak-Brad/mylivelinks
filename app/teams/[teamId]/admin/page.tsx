...
export default function TeamAdminPage() {
  ...
  const teamId = params?.teamId ?? 'team_demo_001';
              ...
              <ModerationTab
                ...
                onMute={(profileId, durationSeconds, reason) => {
                  setSnapshot((s) => {
                    ...
                    const mutedUntil = durationSeconds === null ? null : new Date(Date.now() + durationSeconds * 1000).toISOString();
                    const next = {
                      ...s,
                      members: s.members.map((m) => (m.profile.id === profileId ? { ...m, status: { ...m.status, mutedUntil } } : m)),
                    };
                    if (target) {
                      applyAudit(
                        createAuditEvent({
                ...
                onBan={(profileId, durationSeconds, reason) => {
                  setSnapshot((s) => {
                    if (!s) return s;
                    const target = s.members.find((m) => m.profile.id === profileId);
                    const bannedUntil = durationSeconds === null ? null : new Date(Date.now() + durationSeconds * 1000).toISOString();
                    const next = {
                      ...s,
                      members: s.members.map((m) => (m.profile.id === profileId ? { ...m, status: { ...m.status, bannedUntil } } : m)),
                    };
                    if (target) {
                      applyAudit(
                        createAuditEvent({
