-- ============================================================================
-- COMPLETE FIX FOR PROFILES - RUN THIS ONCE
-- ============================================================================

-- Step 1: Add any missing columns (IF NOT EXISTS = safe to run multiple times)
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS profile_type TEXT DEFAULT 'creator';
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS enabled_modules TEXT[];
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS enabled_tabs TEXT[];
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS button_color TEXT;
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS content_text_color TEXT;
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS ui_text_color TEXT;
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS link_color TEXT;
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS show_top_friends BOOLEAN DEFAULT TRUE;
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS top_friends_title TEXT DEFAULT 'Top Friends';
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS top_friends_avatar_style TEXT DEFAULT 'square';
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS top_friends_max_count INTEGER DEFAULT 8;

-- Step 2: Drop and recreate the RPC function with ALL fields
DROP FUNCTION IF EXISTS get_public_profile_with_adult_filtering(text, uuid, text) CASCADE;

CREATE FUNCTION get_public_profile_with_adult_filtering(
    p_username text,
...
DECLARE
    ...
    v_result JSON;
BEGIN
    -- Get profile data with ALL customization fields
    SELECT 
        p.id,
        p.username,
        p.display_name,
