-- ============================================================================
-- MyLiveLinks Profile System Schema Extensions
-- Modern Profile Customization + Social Features + Stats
-- ============================================================================
-- Run this in Supabase SQL Editor to extend existing schema with:
-- - Profile customization (backgrounds, cards, fonts, colors)
-- - Profile links enhancements (icons, section title)
...
-- Add profile customization columns if they don't exist
DO $$ 
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='profiles' AND column_name='profile_bg_overlay') THEN
        ALTER TABLE profiles ADD COLUMN profile_bg_overlay VARCHAR(50) DEFAULT 'dark-medium'; -- 'none', 'dark-light', 'dark-medium', 'dark-heavy', 'blur'
    END IF;
    
    -- Card styling
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='profiles' AND column_name='card_color') THEN
        ALTER TABLE profiles ADD COLUMN card_color VARCHAR(20) DEFAULT '#FFFFFF'; -- Hex color
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='profiles' AND column_name='card_border_radius') THEN
        ALTER TABLE profiles ADD COLUMN card_border_radius VARCHAR(20) DEFAULT 'medium'; -- 'small', 'medium', 'large'
    END IF;
    
    -- Typography
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='profiles' AND column_name='font_preset') THEN
        ALTER TABLE profiles ADD COLUMN font_preset VARCHAR(20) DEFAULT 'modern'; -- 'modern', 'classic', 'bold', 'minimal'
    END IF;
    
    -- Accent color
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='profiles' AND column_name='accent_color') THEN
        ALTER TABLE profiles ADD COLUMN accent_color VARCHAR(20) DEFAULT '#3B82F6'; -- Hex color (blue default)
    END IF;
    ...
    -- Gifter level (may already exist from gifter system)
...
-- Add icon field to user_links if it doesn't exist
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='user_links' AND column_name='icon') THEN
        ALTER TABLE user_links ADD COLUMN icon VARCHAR(100); -- Icon name or emoji
    END IF;
END $$;

COMMENT ON COLUMN user_links.icon IS 'Icon identifier (emoji or icon name like "instagram", "twitter", etc.)';

-- ============================================================================
-- 3. CREATE STREAM STATS TABLE
-- ============================================================================

-- Stream statistics per user
CREATE TABLE IF NOT EXISTS stream_stats (
    ...
    total_minutes_live BIGINT DEFAULT 0, -- Total minutes broadcasted
    total_viewers BIGINT DEFAULT 0, -- Aggregate viewer count across all streams
    peak_viewers INTEGER DEFAULT 0, -- Highest concurrent viewers in a single session
    
    -- Engagement metrics
    diamonds_earned_lifetime BIGINT DEFAULT 0, -- Total diamonds earned from gifts
    diamonds_earned_7d BIGINT DEFAULT 0, -- Last 7 days
    followers_gained_from_streams INTEGER DEFAULT 0, -- Followers gained while live
...
CREATE INDEX idx_stream_stats_profile_id ON stream_stats(profile_id);
CREATE INDEX idx_stream_stats_diamonds_earned ON stream_stats(diamonds_earned_lifetime DESC);
CREATE INDEX idx_stream_stats_peak_viewers ON stream_stats(peak_viewers DESC);

-- Enable RLS on stream_stats
ALTER TABLE stream_stats ENABLE ROW LEVEL SECURITY;

-- RLS Policy: Everyone can view stats
CREATE POLICY "Stream stats are viewable by everyone"
    ON stream_stats FOR SELECT
    USING (true);

-- RLS Policy: Users can insert/update own stats (or via RPC)
CREATE POLICY "Users can manage own stream stats"
    ON stream_stats FOR ALL
    USING (auth.uid() = profile_id)
    WITH CHECK (auth.uid() = profile_id);

COMMENT ON TABLE stream_stats IS 'Aggregated streaming statistics per user. Updated after each stream session.';

-- ============================================================================
-- 4. CREATE VIEW: FRIENDS (MUTUAL FOLLOWS)
-- ============================================================================

-- Create a view for mutual follows (friends)
...
WHERE f1.follower_id < f1.followee_id; -- Prevent duplicates

COMMENT ON VIEW friends IS 'Mutual follow relationships (friends). Only returns unique pairs.';

-- ============================================================================
-- 5. CREATE RPC: GET PUBLIC PROFILE
-- ============================================================================

-- Single endpoint to fetch complete profile data
CREATE OR REPLACE FUNCTION get_public_profile(
    p_username VARCHAR(50),
    p_viewer_id UUID DEFAULT NULL -- Optional: current user viewing the profile
)
RETURNS JSON AS $$
DECLARE
    v_profile RECORD;
...
BEGIN
    ...
    SELECT 
        ...
        -- Privacy: don't expose balances unless owner
    ...
    -- Determine relationship to viewer
    v_relationship := 'none';
    ...
    -- Get Top 3 Supporters (highest gifters to this profile)
    ...
    -- Get Top 3 Streamers (platform-wide, based on diamonds earned)
    ...
    -- Get stream stats for this profile
...
COMMENT ON FUNCTION get_public_profile IS 'Fetch complete profile data in a single request. Returns profile, links, counts, relationships, and leaderboards.';

-- ============================================================================
-- 6. CREATE RPC: TOGGLE FOLLOW
-- ============================================================================
...
COMMENT ON FUNCTION toggle_follow IS 'Toggle follow/unfollow. Returns new relationship status.';

-- ============================================================================
-- 7. CREATE RPC: GET FOLLOWERS LIST
-- ============================================================================
...
-- ============================================================================
-- 8. CREATE RPC: GET FOLLOWING LIST
-- ============================================================================
...
-- ============================================================================
-- 10. CREATE RPC: TRACK LINK CLICK
-- ============================================================================
...
COMMENT ON FUNCTION track_link_click IS 'Increment click count for a link. Call when user clicks external link.';
...
-- Summary comment
COMMENT ON SCHEMA public IS 'MyLiveLinks Profile System: Extended with customization, social features, stats tracking, and optimized RPC functions for modern profile pages.';
