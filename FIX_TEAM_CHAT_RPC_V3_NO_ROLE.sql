-- ============================================================================
-- NUCLEAR OPTION: Make author_role nullable to bypass type checking
-- ============================================================================
-- If PostgREST won't reload the cache, we'll just make the problematic
-- column nullable so the type mismatch doesn't cause a 400 error
-- ============================================================================

BEGIN;

DROP FUNCTION IF EXISTS public.rpc_get_team_chat_messages_v3(uuid, int, timestamptz, uuid);

CREATE OR REPLACE FUNCTION public.rpc_get_team_chat_messages_v3(
  p_team_id uuid,
...
RETURNS TABLE (
  ...
  -- REMOVED author_role entirely to bypass the type checking bug
)
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION public.rpc_get_team_chat_messages_v3(uuid, int, timestamptz, uuid) TO authenticated;

COMMIT;

SELECT 'V3 function created - author_role removed to bypass type bug' AS result;
