-- ============================================================================
-- COMMENT LIKES & REPLIES SYSTEM - Complete Installation
...
-- ============================================================================
-- PART 1: COMMENT LIKES
-- ============================================================================

-- 1. Add like_count column to post_comments
DO $$ 
BEGIN
  IF NOT EXISTS (
...
-- 9. Trigger to update like_count on INSERT
...
-- 10. Trigger to update like_count on DELETE
...
-- ============================================================================
-- PART 2: COMMENT REPLIES (NESTED COMMENTS)
-- ============================================================================

-- 11. Add parent_comment_id column for nested replies
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
...
-- 12. Add reply_count column to track number of replies
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
...
-- 14. Initialize reply_count for existing comments
UPDATE public.post_comments 
SET reply_count = (
  SELECT COUNT(*) 
...
-- 15. Trigger to increment reply_count when a reply is added
...
-- 16. Trigger to decrement reply_count when a reply is deleted
...
WHERE table_schema = 'public'
  AND table_name = 'post_comments'
  AND column_name IN ('like_count', 'reply_count', 'parent_comment_id')
...
WHERE routine_schema = 'public'
  AND routine_name IN ('like_comment', 'unlike_comment')
...
BEGIN
  ...
  RAISE NOTICE '   - Comment likes with heart icon';
  RAISE NOTICE '   - Nested replies (Facebook-style)';
  RAISE NOTICE '   - Auto-updating like/reply counts';
  RAISE NOTICE '   - RPC functions for atomic operations';
END $$;
