...
-- Step 1: Ensure idempotency_key has a UNIQUE constraint
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE NOTICE '✓ Added UNIQUE constraint on ledger_entries.idempotency_key';
  ELSE
    RAISE NOTICE '✓ UNIQUE constraint already exists on ledger_entries.idempotency_key';
  END IF;
END $$;

-- Step 2: Re-create send_gift_v2 with the ON CONFLICT clause
-- This ensures the function uses the correct syntax
...
BEGIN
  ...
  -- ⚠️ CRITICAL: Validate recipient exists
  ...
  -- Get gift rate from config (1:1 by default)
  ...
  ELSE
    -- Fallback to first active gift type
    SELECT id
    INTO v_gift_type_id
  ...
  -- Insert sender ledger entry (coins spent)
  ...
  -- Insert recipient ledger entry (diamonds earned)
  INSERT INTO public.ledger_entries (
    idempotency_key,
    user_id,
    entry_type,
...
BEGIN
  ...
  ELSE
    RAISE EXCEPTION '✗ Constraint verification failed - idempotency_key constraint missing';
  END IF;
  ...
  IF v_function_def LIKE '%ON CONFLICT (idempotency_key)%' THEN
    RAISE NOTICE '✓ Function verification passed - ON CONFLICT clause present';
  ELSE
    RAISE WARNING '⚠ Function might not have ON CONFLICT clause';
  END IF;

  RAISE NOTICE '✓ Gift sending fix applied successfully';
END $$;
