...
export default function CreatorStudioMoviesScreen() {
  const navigation = useNavigation<any>();
  const { colors } = useTheme();
  const [activeFilter, setActiveFilter] = useState<FilterChip>('all');
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [content, setContent] = useState<MovieItem[]>([]);

  const fetchContent = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      // API endpoint: GET /api/creator-studio/movies
  ...
  const renderItem = ({ item }: { item: MovieItem }) => {
    ...
    return (
      <Pressable
        ...
        onPress={() => navigation.navigate('CreatorStudioItemDetailScreen', { itemId: item.id, type: 'movie' })}
        style={({ pressed }) => [
          styles.contentCard,
      ...
      >
        {/* Thumbnail */}
        <View style={[styles.thumbnail, { backgroundColor: 'rgba(139,92,246,0.1)' }]}>
          <Ionicons name="film-outline" size={24} color="#8B5CF6" />
        </View>
        ...
        <View style={styles.contentInfo}>
          ...
          <View style={styles.metaRow}>
            <View style={[styles.durationBadge, { backgroundColor: 'rgba(139,92,246,0.15)' }]}>
    ...
    <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.bg }]} edges={['top', 'left', 'right', 'bottom']}>
      ...
      <View style={[styles.header, { borderBottomColor: colors.border }]}>
        <View style={styles.headerLeft}>
          ...
          <Text style={[styles.headerTitle, { color: colors.text }]}>Movies (Long Form)</Text>
        </View>
        <Pressable
          accessibilityRole="button"
        ...
        {FILTER_CHIPS.map((chip) => {
          ...
          return (
            <Pressable
              ...
              style={[
                styles.filterChip,
                { backgroundColor: isActive ? '#8B5CF6' : colors.surface, borderColor: isActive ? '#8B5CF6' : colors.border },
              ]}
            >
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyState}>
            <Ionicons name="film-outline" size={48} color={colors.mutedText} />
            <Text style={[styles.emptyTitle, { color: colors.text }]}>No movies found</Text>
            <Text style={[styles.emptySubtitle, { color: colors.mutedText }]}>
              Upload your first long-form content
            </Text>
          </View>
        }
      />
...
const styles = StyleSheet.create({
  ...
  durationBadgeText: { fontSize: 10, fontWeight: '800', color: '#8B5CF6' },
  statusBadge: {
    paddingHorizontal: 8,
    paddingVertical: 3,
    borderRadius: 999,
