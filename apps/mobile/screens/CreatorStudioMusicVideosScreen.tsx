...
export default function CreatorStudioMusicVideosScreen() {
  const navigation = useNavigation<any>();
  const { colors } = useTheme();
  const [activeFilter, setActiveFilter] = useState<FilterChip>('all');
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [content, setContent] = useState<MusicVideoItem[]>([]);

  const fetchContent = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      // API endpoint: GET /api/creator-studio/music-videos
      // Will be implemented by web team
      setContent([]);
    } catch (e: any) {
      setError(e?.message || 'Failed to load music videos');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const renderItem = ({ item }: { item: MusicVideoItem }) => {
    const statusConfig = STATUS_CONFIG[item.status];

    return (
      <Pressable
        accessibilityRole="button"
        onPress={() => navigation.navigate('CreatorStudioItemDetailScreen', { itemId: item.id, type: 'music_video' })}
        style={({ pressed }) => [
          styles.contentCard,
      ...
      >
        {/* Thumbnail */}
        <View style={[styles.thumbnail, { backgroundColor: 'rgba(236,72,153,0.1)' }]}>
        ...
        <View style={styles.contentInfo}>
          ...
          <View style={styles.metaRow}>
            <View style={[styles.durationBadge, { backgroundColor: 'rgba(236,72,153,0.15)' }]}>
    ...
    <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.bg }]} edges={['top', 'left', 'right', 'bottom']}>
      ...
      <View style={[styles.header, { borderBottomColor: colors.border }]}>
        <View style={styles.headerLeft}>
          <View style={[styles.headerIcon, { backgroundColor: '#EC4899' }]}>
            <Ionicons name="musical-notes-outline" size={18} color="#FFFFFF" />
          </View>
          <Text style={[styles.headerTitle, { color: colors.text }]}>Music Videos</Text>
        </View>
        <Pressable
          accessibilityRole="button"
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyState}>
            <Ionicons name="musical-notes-outline" size={48} color={colors.mutedText} />
            <Text style={[styles.emptyTitle, { color: colors.text }]}>No music videos found</Text>
...
const styles = StyleSheet.create({
  ...
  durationBadgeText: { fontSize: 10, fontWeight: '800', color: '#EC4899' },
  statusBadge: {
    paddingHorizontal: 8,
    paddingVertical: 3,
    borderRadius: 999,
