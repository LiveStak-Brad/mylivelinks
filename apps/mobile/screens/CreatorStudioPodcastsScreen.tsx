...
export default function CreatorStudioPodcastsScreen() {
  const navigation = useNavigation<any>();
  const { colors } = useTheme();
  const [activeFilter, setActiveFilter] = useState<FilterChip>('all');
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [content, setContent] = useState<PodcastItem[]>([]);

  const fetchContent = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      // API endpoint: GET /api/creator-studio/podcasts
  ...
  const renderItem = ({ item }: { item: PodcastItem }) => {
    ...
    return (
      <Pressable
        ...
        onPress={() => navigation.navigate('CreatorStudioItemDetailScreen', { itemId: item.id, type: 'podcast' })}
        style={({ pressed }) => [
          styles.contentCard,
      ...
      >
        {/* Thumbnail */}
        <View style={[styles.thumbnail, { backgroundColor: 'rgba(245,158,11,0.1)' }]}>
        ...
        <View style={styles.contentInfo}>
          ...
          <View style={styles.metaRow}>
            <View style={[styles.durationBadge, { backgroundColor: 'rgba(245,158,11,0.15)' }]}>
          ...
          <View style={styles.statsRow}>
            <Ionicons name="headset-outline" size={12} color={colors.mutedText} />
            <Text style={[styles.statsText, { color: colors.mutedText }]}>
              {item.views.toLocaleString()} listens
            </Text>
          </View>
        </View>
    ...
    <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.bg }]} edges={['top', 'left', 'right', 'bottom']}>
      ...
      <View style={[styles.header, { borderBottomColor: colors.border }]}>
        <View style={styles.headerLeft}>
          <View style={[styles.headerIcon, { backgroundColor: '#F59E0B' }]}>
            <Ionicons name="mic-outline" size={18} color="#FFFFFF" />
          </View>
          <Text style={[styles.headerTitle, { color: colors.text }]}>Podcasts</Text>
        </View>
        <Pressable
          accessibilityRole="button"
        ...
        {FILTER_CHIPS.map((chip) => {
          ...
          return (
            <Pressable
              ...
              style={[
                styles.filterChip,
                { backgroundColor: isActive ? '#F59E0B' : colors.surface, borderColor: isActive ? '#F59E0B' : colors.border },
              ]}
            >
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyState}>
            <Ionicons name="mic-outline" size={48} color={colors.mutedText} />
            <Text style={[styles.emptyTitle, { color: colors.text }]}>No podcasts found</Text>
...
const styles = StyleSheet.create({
  ...
  durationBadgeText: { fontSize: 10, fontWeight: '800', color: '#F59E0B' },
  statusBadge: {
    paddingHorizontal: 8,
    paddingVertical: 3,
    borderRadius: 999,
