-- ============================================================================
-- V4 FUNCTION - PostgREST Cache Bypass (RECOMMENDED SOLUTION)
-- ============================================================================
-- Purpose: Create a new V4 function with identical logic to V3 but new name
--          to bypass PostgREST's persistent schema caching issue
-- ============================================================================
-- Run this if DIAGNOSE_TEAM_CHAT_V3.sql shows the function works in SQL
-- but still fails from the client (42804 error in browser)
-- ============================================================================

BEGIN;

-- Drop existing V4 if it exists
DROP FUNCTION IF EXISTS public.rpc_get_team_chat_messages_v4(uuid, int, timestamptz, uuid);

-- Create V4 with NO author_role column (identical to V3 but new name)
CREATE OR REPLACE FUNCTION public.rpc_get_team_chat_messages_v4(
  p_team_id uuid,
...
RETURNS TABLE (
  ...
  -- NO author_role - removed to bypass type mismatch
)
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  -- Return messages with author info
  RETURN QUERY
  SELECT
    m.id AS message_id,
    m.team_id,
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION public.rpc_get_team_chat_messages_v4(uuid, int, timestamptz, uuid) TO authenticated;
...
SELECT 'V4 function created successfully - Update client to use v4' AS result;

-- ============================================================================
-- NEXT STEP: Update hooks/useTeam.ts line 833
-- ============================================================================
-- Change:
