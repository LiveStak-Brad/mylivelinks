...
-- Add asset_type column to coin_ledger
...
UPDATE coin_ledger SET asset_type = 'coin' WHERE asset_type IS NULL;
...
ALTER TABLE coin_ledger 
DROP CONSTRAINT IF EXISTS coin_ledger_type_check;

ALTER TABLE coin_ledger 
ADD CONSTRAINT coin_ledger_type_check 
CHECK (type IN (
...
CREATE INDEX IF NOT EXISTS idx_coin_ledger_asset_type ON coin_ledger(asset_type);
CREATE INDEX IF NOT EXISTS idx_coin_ledger_profile_asset ON coin_ledger(profile_id, asset_type);

COMMENT ON COLUMN coin_ledger.asset_type IS 'Asset type: coin (purchased) or diamond (earned from gifts)';
...
CREATE TABLE IF NOT EXISTS diamond_conversions (
    ...
    conversion_rate DECIMAL(5, 4) NOT NULL DEFAULT 0.7000, -- 70% (1 - 0.30 fee)
...
COMMENT ON TABLE diamond_conversions IS 'Diamond to coin conversions with 30% platform fee. coins_out = floor(diamonds_in * 0.70), fee_amount = diamonds_in - coins_out.';
