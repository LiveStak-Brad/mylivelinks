-- Add playlists to enabled_tabs constraint
-- This is a new profile tab for curator playlists (YouTube video collections)
...
-- Add new check constraint with ALL valid tab names including playlists
...
'Validates enabled_tabs contains only valid tab names from ProfileTab type (includes playlists)';

-- Update the set_enabled_tabs RPC to allow the new tab value
