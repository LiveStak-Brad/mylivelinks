-- ============================================================================
-- MyLiveLinks: Optimize get_profile_with_context RPC
-- ============================================================================
-- Removes expensive COUNT queries and uses cached columns instead.
-- This eliminates 2-3 COUNT queries on every profile load.
-- ============================================================================

BEGIN;

-- ============================================================================
-- UPDATE get_profile_with_context TO USE CACHED COUNTS
-- ============================================================================

CREATE OR REPLACE FUNCTION public.get_profile_with_context(
    p_username text,
...
BEGIN
    ...
    -- Use cached counts from profile (OPTIMIZED - no more COUNT queries!)
    v_follower_count := COALESCE(v_profile.follower_count, 0);
    v_following_count := COALESCE(v_profile.following_count, 0);
    v_friends_count := COALESCE(v_profile.friends_count, 0);
...
COMMENT ON FUNCTION public.get_profile_with_context IS 'Returns complete profile context with cached counts (optimized - no COUNT queries)';
...
-- ============================================================================
-- What changed:
-- ✅ Removed 3 expensive COUNT queries:
--    - SELECT COUNT(*) FROM follows WHERE followee_id = ... (follower_count)
--    - SELECT COUNT(*) FROM follows WHERE follower_id = ... (following_count)
--    - SELECT COUNT(*) FROM friends WHERE ... (friends_count)
--
-- ✅ Now uses cached columns from profiles table:
--    - v_follower_count := COALESCE(v_profile.follower_count, 0)
--    - v_following_count := COALESCE(v_profile.following_count, 0)
--    - v_friends_count := COALESCE(v_profile.friends_count, 0)
--
-- ✅ Added following_count and friends_count to SELECT query
--
-- Result: Profile loads are now significantly faster and counts are
-- guaranteed to be consistent across all UI components.
-- ============================================================================
