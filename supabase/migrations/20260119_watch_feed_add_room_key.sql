-- ============================================================================
-- Add room_key to rpc_get_watch_feed for group live routing
-- ============================================================================
-- This allows the watch feed to provide room_key/slug for group lives
-- so clients can route to the correct group room instead of defaulting to solo
-- ============================================================================
...
RETURNS TABLE (
  ...
  item_type text,
  post_id uuid,
  streaming_mode text,  -- 'solo' | 'group' for live streams, NULL for videos
  room_key text,        -- NEW: room key/slug for group lives (e.g., 'live_central')
  title text,
  caption text,
  hashtags text[],
  location_text text,
...
AS $$
  ...
  video_posts AS (
    SELECT
      ...
      NULL::text AS room_key,        -- NULL for videos
      p.title,
  ...
  live_streams AS (
    SELECT
      ...
      COALESCE(ls.streaming_mode, 'group')::text AS streaming_mode,
      -- NEW: Provide room_key for group lives
      -- For group lives, default to 'live_central' (the main group room)
      -- For solo lives, we don't need room_key (routing uses username instead)
      CASE 
        WHEN COALESCE(ls.streaming_mode, 'group') = 'group' THEN 'live_central'
        ELSE NULL
      END AS room_key,
  ...
  SELECT
    ...
    post_id,
    streaming_mode,
    room_key,  -- NEW: Include in final SELECT
    title,
    caption,
    hashtags,
    location_text,
