-- ============================================================================
-- Exclude owner profile IDs from all leaderboards
-- ============================================================================
-- Owner should not appear on leaderboards to keep competition fair
-- Owner IDs: 2b4a1178-3c39-4179-94ea-314dd824a818, 0b47a2d7-43fb-4d38-b321-2d5d0619aabf
-- ============================================================================

BEGIN;

-- Drop existing function to recreate with owner exclusion
DROP FUNCTION IF EXISTS public.get_leaderboard(text, text, integer, text);

-- Create updated function that excludes owner profile IDs
  ...
  IF LOWER(COALESCE(p_type, '')) IN ('top_gifters', 'gifters', 'gifter') THEN
    ...
    WITH sums AS (
      ...
      WHERE le.entry_type IN ('coin_spend_gift', 'coin_spend_team_pool_gift')
        ...
        -- Exclude owner profile IDs
        AND le.user_id != ALL(v_owner_ids)
      GROUP BY le.user_id
    )
    SELECT
