-- Add display_photo_preference to rpc_get_teams_discovery_ordered
-- This allows home page and teams page to respect team admin's photo preference

-- Drop the existing function first since we're changing the return type
DROP FUNCTION IF EXISTS public.rpc_get_teams_discovery_ordered(int, int);
...
'Returns teams ordered by discovery priority with display_photo_preference: Photo+Members > Photo+NoMembers > NoPhoto+Members > NoPhoto+NoMembers. Within each tier, sorted by member count DESC, then created_at DESC.';
