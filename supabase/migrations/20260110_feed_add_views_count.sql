-- Migration: Add views_count to get_public_feed RPC
-- This allows the feed to display view counts alongside likes
...
-- Recreate with views_count field
...
AS $$
  WITH base_posts AS (
    SELECT p.id, p.author_id, p.text_content, p.media_url, p.created_at, p.visibility, p.is_pinned, p.pinned_at, p.views_count
    FROM public.posts p
