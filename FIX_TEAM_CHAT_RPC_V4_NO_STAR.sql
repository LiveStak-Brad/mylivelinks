-- ============================================================================
-- FIX v4: EXPLICIT column selection (no SELECT *)
-- ============================================================================
-- The problem: wa.* passes the original enum column forward
-- The solution: Explicitly name every column we need
-- ============================================================================

BEGIN;

DROP FUNCTION IF EXISTS public.rpc_get_team_chat_messages_v2(uuid, int, timestamptz, uuid);
...
BEGIN
  ...
  with_authors AS (
    SELECT
      ...
      p.display_name,
      p.avatar_url,
      COALESCE(tm.role::text, 'Team_Member') as role_text  -- Explicitly cast and rename
    FROM messages msg
  ...
  with_replies AS (
    SELECT
      ...
      wa.display_name,
      wa.avatar_url,
      wa.role_text,  -- Now only text type, no enum
      CASE
  ...
  SELECT
    ...
    wr.role_text AS author_role,  -- Pure text, no casting needed
...
SELECT 'V4 function created - NO MORE SELECT *' AS result;
