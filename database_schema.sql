...
CREATE TABLE profiles (
    ...
    -- Coin balances (cached - source of truth is coin_ledger)
...
CREATE INDEX idx_coin_ledger_type ON coin_ledger(type);
CREATE INDEX idx_coin_ledger_ref ON coin_ledger(ref_type, ref_id);
CREATE INDEX idx_coin_ledger_created_at ON coin_ledger(created_at DESC);

-- Enable RLS on coin_ledger
...
-- All coin movements must go through coin_ledger first
...
COMMENT ON TABLE coin_ledger IS 'Immutable transaction log - source of truth for coin balances. profiles.coin_balance is cached.';
