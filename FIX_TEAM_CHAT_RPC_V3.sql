-- ============================================================================
-- FIX v3: rpc_get_team_chat_messages - Alternative fix using ::text
-- ============================================================================
-- This version uses PostgreSQL's native :: casting which is more reliable
-- ============================================================================

BEGIN;

-- Drop ALL versions of the function (in case of signature mismatch)
DROP FUNCTION IF EXISTS public.rpc_get_team_chat_messages(uuid, int, timestamptz, uuid);
DROP FUNCTION IF EXISTS public.rpc_get_team_chat_messages(uuid, int, timestamptz);
DROP FUNCTION IF EXISTS public.rpc_get_team_chat_messages(uuid, int);
DROP FUNCTION IF EXISTS public.rpc_get_team_chat_messages(uuid);
...
BEGIN
  ...
  SELECT
    ...
    wr.role_text AS author_role,  -- FIX: Now using pre-cast text field
...
-- Verify it worked
SELECT 
  'Team chat RPC fixed (v3) - enum cast moved to CTE' AS result,
  routine_name,
  created
